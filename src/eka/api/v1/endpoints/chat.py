...
from eka.deps import get_rag_pipeline, get_user_by_token
...
async def chat(
    ...
    pipeline: RAGQueryPipeline = Depends(get_rag_pipeline),
):
    """
    ...
    answer = await pipeline.chat(query=query, tenant_id=current_user.tenant_id)
...
async def chat_stream(
    ...
    async def event_generator():
        try:
            async for token in pipeline.chat_stream(query=query, tenant_id=current_user.tenant_id):
